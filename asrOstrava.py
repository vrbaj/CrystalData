...
# data creation
data_series = np.load('filtrovana_data.npy')
print(data_series.shape)
# data_series = np.array([0., 1., 2., 3., 4., 6])
plt.figure(1)
...
for i in range(gev_window, dw.shape[0]):
    ...
    for j in range(n):
        ...
        hpp[i-gev_window, j] = genextreme.pdf(dw[i, j], fit[0], loc=fit[1], scale=fit[2])*fit[2]
...
# print('FIT2:', fit2)
# hpp2 = genextreme.pdf(dw[10:13000], fit2[0], loc=fit2[1], scale=fit2[2])*fit2[2]
# print('odhad hpp2:')
